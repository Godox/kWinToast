...
import icu.ootime.jwintoast.WinToast.Companion.instance
...
//        dd.toastActivated(1);
        try {
            ...
            winToast!!.setAppName(CharPointer("测试"))
...
//            winToastTemplate.addAction(new CharPointer("否"));
    ...
    class MyToastController() : WinToastController("load") {
        lateinit var progressBar : WinNotifProgressBar
    }

    @Test
    fun `test from xml`() {
//        val toast = NotificationManager.loadFromFile(, "load.xml")
        val myToastController = MyToastController()
        myToastController.show()
        myToastController.progressBar.status
